...
class SequentialJointPipeline(BasePipeline):
    '''
    ...
    def run(self, train_dataset, validation_dataset):
        ...
        first_phase_sr_seg_path = os.path.join(self.config.folder_name, f'{self.config.trainable_sr_frozen_seg}.pth')
        load_model_for_inference(model=first_phase_sr_seg, saving_name=first_phase_sr_seg_path)
    ...
    def test(self, test_dataset):
        ...
        sequential_joint_model = MultiStageModel(
            sr_model, 
            seg_model, 
            freeze_stage_1=True, 
            freeze_stage_2=True
        )
        load_model_for_inference(model=sequential_joint_model, saving_name=self.saving_path)

        trainer = MultiStageTrainer(
            config=self.config,
            model=sequential_joint_model,
            device=self.device,
            validation_metrics=validation_metrics
        )
    ...
    def predict(self, input_tensor):
        ...
        load_model_for_inference(model=sequential_joint_model, saving_name=self.saving_path)
        sequential_joint_model.to(self.device).eval()
        ...
        with torch.no_grad():
            output_mask, _ = sequential_joint_model(input_image)
